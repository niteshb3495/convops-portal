...
const SCENARIOS = [
  {
    ...
    investigationSteps: [
      ...
      "Correlating memory metrics…",
      "Identifying failing container…",
    ],
    rootCause:
      "Container api-worker-3 hit OOM limit (512MB). Memory usage spiked from 340MB → 620MB over 8 minutes, triggering 500 errors. 1 of 4 tasks in STOPPED state.",
    explanation:
      "A memory leak in the latest deploy (v2.4.1, 23 min ago) is causing the api-worker containers to exhaust their memory limit. The stopped task is not being replaced because ECS health check grace period has expired.",
    suggestedFix: "Restart the ECS service to cycle the faulty container and restore all 4 tasks to RUNNING state.",
    action: "Restart ECS service — api-service",
    resolvedMessage: "✅ ECS service restarted. All 4 tasks now RUNNING. ErrorRate back to 0.1%. Incident resolved in 2 minutes.",
  },
  {
    id: "lambda-timeout",
    title: "Lambda — Silent Timeout",
    alarm: "payment-processor · Duration > 28s · 5 invocations",
    account: "prod-account (123456789012)",
    region: "eu-central-1",
    severity: "P2",
    investigationSteps: [
      "Fetching Lambda execution logs…",
      "Checking downstream dependencies…",
      "Analysing timeout patterns…",
      "Correlating with RDS metrics…",
    ],
    rootCause:
      "Lambda is timing out waiting on RDS connection pool (avg 27.8s, limit 30s). RDS CPU at 94%, connection count at 487/500. Pool exhausted.",
    explanation:
      "The payment-processor Lambda is holding connections open too long due to missing connection.release() in the error path (introduced in v1.9.2). RDS is now saturated — all new Lambda invocations queue for a connection and hit the 30s timeout.",
    suggestedFix: "Reboot RDS instance to clear stale connections and restore the connection pool.",
    action: "Reboot RDS instance — payments-db",
    resolvedMessage: "✅ RDS rebooted. Connection count back to 12/500. Lambda avg duration 340ms. Payment processing restored.",
  },
];
